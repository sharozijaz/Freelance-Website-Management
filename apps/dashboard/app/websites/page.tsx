...
export default async function WebsitesPage({
  ...
  if (!context) {
    return (
      <DashboardPage title="Websites">
        <UnauthorizedState message="Sign in to view websites." />
      </DashboardPage>
    );
  }
  ...
  return (
    <DashboardPage
      description="Operational website inventory across accessible workspaces."
      title="Websites"
    >
      <FilterBar
            ...
            <form action="/api/websites" className="grid gap-3 xl:grid-cols-4" method="post">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {websiteTypeDescriptions.external_legacy}
                </p>
              </div>
              <div>
      ...
      {websites.items.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-surface">
          <div className="hidden grid-cols-[1.1fr_0.9fr_0.8fr_0.7fr_0.8fr_1fr_0.7fr_auto] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase text-muted-foreground md:grid">
            <span>Website</span>
            <span>Client</span>
            <span>Type</span>
            <span>Status</span>
          ...
          {websites.items.map((website) => (
            <div
              className="grid gap-2 border-b border-border p-4 last:border-b-0 md:grid-cols-[1.1fr_0.9fr_0.8fr_0.7fr_0.8fr_1fr_0.7fr_auto] md:items-center"
              key={website.id}
            >
              <div>
                <p className="font-medium">{website.name}</p>
                {website.productionUrl ? (
                  <Button asChild className="mt-1 px-0" size="sm" variant="ghost">
                    <a href={website.productionUrl}>View website</a>
                  </Button>
                ) : null}
              </div>
              <span className="text-sm">{website.organizationName}</span>
              <span className="text-sm">{websiteTypeLabels[website.websiteType]}</span>
              <Badge variant={website.status === "active" ? "success" : "outline"}>
                {website.status}
              </Badge>
              <Badge variant={website.deploymentStatus === "failed" ? "error" : "outline"}>
                {website.deploymentStatus}
              </Badge>
              <span className="text-sm text-muted-foreground">
                {website.primaryDomain ?? "No primary domain"}
              </span>
              <span className="text-sm text-muted-foreground">
