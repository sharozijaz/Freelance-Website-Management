...
export default async function DomainsPage({
  ...
  if (!context) {
    return <DashboardPage title="Domains"><UnauthorizedState message="Sign in to view domains." /></DashboardPage>;
  }
      ...
      {domains.items.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-surface">
          <div className="hidden grid-cols-[1fr_0.9fr_0.8fr_0.7fr_0.7fr_0.6fr_auto] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase text-muted-foreground md:grid">
            <span>Domain</span><span>Website</span><span>Client</span><span>DNS</span><span>SSL</span><span>Primary</span><span>Open</span>
          </div>
          {domains.items.map((domain) => (
            <div className="grid gap-2 border-b border-border p-4 last:border-b-0 md:grid-cols-[1fr_0.9fr_0.8fr_0.7fr_0.7fr_0.6fr_auto] md:items-center" key={domain.id}>
              <div>
                <p className="font-medium">{domain.domain}</p>
                <Badge className="mt-1" variant={domain.verificationStatus === "verified" ? "success" : "warning"}>
                  {domain.verificationStatus}
                </Badge>
              </div>
              <Link className="text-sm underline-offset-4 hover:underline" href={`/websites/${domain.websiteId}/hosting`}>
                {domain.websiteName}
              </Link>
              <span className="text-sm">{domain.organizationName}</span>
              <Badge variant={domain.dnsState === "valid" ? "success" : "outline"}>{domain.dnsState}</Badge>
              <Badge variant={domain.sslState === "issued" ? "success" : "outline"}>{domain.sslState}</Badge>
              <span className="text-sm text-muted-foreground">{domain.isPrimary ? "Yes" : "No"}</span>
