...
import { listPlatformBlogCategories } from "@/lib/platform-api/blog";
...
async function getPlatformBlogCategoriesResponse({
  ...
  try {
    ...
    const data = await listPlatformBlogCategories({ context, database: databaseClient });
...
export async function GET(request: Request) {
  return getPlatformBlogCategoriesResponse({ database, request });
}
