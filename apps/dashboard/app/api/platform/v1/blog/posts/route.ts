...
import { listPlatformBlogPosts } from "@/lib/platform-api/blog";
...
function stringParam(searchParams: URLSearchParams, name: string) {
...
async function getPlatformBlogPostsResponse({
  ...
  try {
    ...
    const data = await listPlatformBlogPosts({
      context,
      database: databaseClient,
      options: {
        category: stringParam(url.searchParams, "category"),
        limit: positiveIntegerParam(url.searchParams, "limit"),
        page: positiveIntegerParam(url.searchParams, "page"),
        preview: previewParam(request),
        tag: stringParam(url.searchParams, "tag"),
      },
    });
...
export async function GET(request: Request) {
  return getPlatformBlogPostsResponse({ database, request });
}
